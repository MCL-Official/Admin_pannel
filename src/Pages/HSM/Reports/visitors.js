...
import { HSM_allProduct, getallVisitors } from "../../User_Management/features/userSlice";
...
const Visitors = ({ setActiveTab, setExpand }) => {
...
// console.log(ProductCategoriees[0]?.SEOArea,"herasdfuyg");
  ...
  const getVisitorsData = useSelector((state) => state?.userManagement?.getallVisitors);
  // console.log(getVisitorsData,"viis")
  useEffect(() => {
    const fetchUserData = async () => {
      setLoading(true);
      await dispatch(getallVisitors());
  ...
  let count = 1;
  let visitorsCountry = [...new Set(getVisitorsData.map((item) => item.address.country))];
  let visitorsRegion  = [...new Set(getVisitorsData.map((item) => item.address.region))];
  
  // console.log("lkjh",visitorsCountry)
  // console.log("lkjhg",visitorsRegion)

  const data = getVisitorsData.map((user) => {
    let cat;
      ...
      <div className="   ml-72 mt-28 w-[75vw] relative">
        {productData?.length > 0 ? (
          <Table
            ...
            catgoryFilterRegion={visitorsRegion}
            catgoryFilterCountry={visitorsCountry}
          />
        ) : (
          <>
            <Table
