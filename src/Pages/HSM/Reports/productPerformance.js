...
const ProductPerformance = ({ setActiveTab, setExpand }) => {
  ...
  const PerformanceData = useSelector(
    (state) => state?.userManagement?.getProductPerformance
  );
  // console.log("PerformanceData", PerformanceData);
  useEffect(() => {
    const fetchUserData = async () => {
      setLoading(true);
      await dispatch(getProductPerformance());
  ...
  const productStatus = [...new Set(PerformanceData.map((item) => item.status))];


  // console.log("nal", productStatus);
  const data = PerformanceData.map((user) => {
    let cat;
    ...
    return {
      ...
      price: user?.price,
      soldQty: user?.soldQty,
      lastSoldDate: user?.lastSoldDate,
      information: (
        <Info
          sale={user?.sales}
      ...
      <div className="   ml-72 mt-28 w-[75vw] relative">
        {PerformanceData?.length > 0 ? (
          <Table
            ...
            pageSize={pageSize}
            blackButtonText={
              <a href={`${tssurl}/product/export-product-performance`}>
                {blackButtonText}
              </a>
            }
