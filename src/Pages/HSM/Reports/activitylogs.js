...
import { HSM_allProduct, getactivitylogs } from "../../User_Management/features/userSlice";
...
const ActivityLogs = ({ setActiveTab, setExpand }) => {
...
// console.log(ProductCategoriees[0]?.SEOArea,"herasdfuyg");
  const LuserData = useSelector((state) => state?.userManagement?.getUserLogin);
  const ActivityLogsData = useSelector(
    (state) => state.userManagement.getactivitylogs
    );
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      setLoading(true);
      await dispatch(getactivitylogs())
  ...
  const columns = [
    ...
    {
      header: "Singup Count",
      accessor: "Singup_Count",
    }
  ];
  // console.log(productData);
  let count = 1;
  let productCategory1 = ActivityLogsData.map((item) => { return (item?.item_category == 'prodcat') ? 'Product' : 'Service' });
  const data = ActivityLogsData.map((user) => {
    let cat;
    ...
    return ({
      serialno: count++,
      date: user?.date,
      Login_Count: user?.loginCounts,
      Singup_Count: user?.signupCount,
      information: (
        <Info
          sale={user?.sales}
