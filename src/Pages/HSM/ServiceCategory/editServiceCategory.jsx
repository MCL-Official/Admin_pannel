...
import { serviceCategoryList } from "../../User_Management/features/userSlice";
import { Link, useNavigate } from "react-router-dom";
import { Grid } from "react-loader-spinner";

const EditServiceCategory = ({ setExpand, setActiveTab }) => {
    setExpand("homeService");
    setActiveTab("serviceCategoryList");
    const head = "Edit Serivce Category";
    const navigate = useNavigate();
    const location = useLocation();
    const data = location.state;
    // console.log(data.CategoryList);
    const [scid, setpcid] = useState(data.scid);
    ...
    const handleSubmit = async (event) => {
        ...
        setLoading(true);
        await dispatch(serviceCategoryList({ formData, title }));
        setLoading(false);
